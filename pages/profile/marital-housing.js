...
export default function MaritalHousing() {
  const { maritalHousing } = profile_data;

  const namesOfInputs = getNames(maritalHousing.fields);
      ...
      >
        {({ values }) => (
          <Form className="h-100 d-flex flex-column profile-form box">
            <Row>
              {maritalHousing.fields.map((field, index) => (
